        ...
        try:
            if action == "chat":
                ...
                coro = global_client.analyze_symptoms(
                    kwargs.get("symptoms", ""), kwargs.get("age"), kwargs.get("gender")
                )
            ...
            response_text, error_message = future.result(
                timeout=30
            )  # 30 second timeout

            # Process special response formats
            if (
                action in ["get_greeting", "get_server_info"]
                and response_text
                and isinstance(response_text, dict)
            ):
                response_text = response_text.get("contents", [{}])[0].get(
...
def parse_message_for_action(message: str) -> tuple[str, dict]:
    ...
    if any(keyword in message_lower for keyword in medical_keywords):
        ...
        if any(
            ...
            for phrase in ["i have", "experiencing", "feeling", "symptoms"]
        ):
...
def chat():
    ...
    if error:
        ...
        session["history"].append(
            {"role": "error", "content": "No response received from the backend."}
        )
...
def get_summary():
    ...
    if error:
        ...
        session["history"].append(
            {"role": "assistant", "content": "📋 No conversation to summarize yet."}
        )
...
def analyze_symptoms():
    ...
    if error:
        ...
        session["history"].append(
            {
                "role": "error",
                "content": "No response received from the medical analysis.",
            }
        )
...
def server_info():
    ...
    if error:
        ...
        session["history"].append(
            {"role": "assistant", "content": f"ℹ️ **Server Information:**\n\n{info}"}
        )
    else:
        session["history"].append(
            {"role": "assistant", "content": "ℹ️ Server information not available."}
        )
...
def get_greeting(name):
    ...
    if error:
        ...
        session["history"].append(
            {
                "role": "assistant",
                "content": f"👋 Hello {name}! Welcome to the Medical Assistant.",
            }
        )
...
def api_chat():
    ...
    return jsonify({"response": bot_response, "action": action, "success": True})
...
def api_analyze():
    ...
    return jsonify({"analysis": bot_response, "success": True})
